...
void loop() {
  ...
  buttonHandler(STATUS_PIN,   modeButtonState,    prevModeButtonState,    currentButtonReferenceTime, modeButtonPrevPush);
}
// ======================================== Button Handler ==================================== //
...
void cathodePoison() {
  ...
  if ((millis() - poisonCycleLast) >= POISON_DWELL) {
    poisonCycleDigit++;
...
//  _delay_us(10);
  PORTD |= (1 << currentDigit);                // Set Cathodes DONT CLOBBER COLON   
...
void timer() {
  ...
  counterHoursCopy = counterHours;              // Variable used for 12 hour mode
  ...
  if (currentMode != TIME_MODE) {              // If system in other mode, system timer needs to count
    systemTimer++;
  }
}
// ========================================= Timer Buffer Update =============================== //
