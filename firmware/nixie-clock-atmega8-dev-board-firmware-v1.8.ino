...
//                                                    Test software version 1.8                                                          //
//                                               Used in strip board protoype for ISP                                                    //
// ------------------------------------------------------------------------------------------------------------------------------------- //

// Todo List:
// Reduce amount of buttons. Use one button for multi function. Double click, hold, etc.
// Add feature to remember hour mode between full power downs. Kinda not needed as a flat backup battery would mean the time needs reset anyway
...
const byte HV_POWER =                            A3;                  // Debug LED
const byte MODE_PIN =                            A4;                  // 24 or 12 hour mode selection
const byte PWR_PIN =                             A5;                  // Pin to monitor if main power is removed
const byte RUN_TIME_HOURS =                      20;                  // EEPROM address for nixie tube run time, 4 bytes 0 - 3
const byte EEPROM_INITIALISED =                  24;                  // EEPROM address to check if EEPROM is initialised
const byte INVALID_DATA =                        10;                  // Used for clear reference to CD4511 invalid data
const byte DIGIT_DWELL_TIME =                    50;                  // Digit dwell time for cathode poisoning function
const byte CATHODE_POISONING_REPEAT =             4;                  // How many digits to cycle in cathode poisoning function
const byte DEBOUNCE_DELAY =                     150;                  // Time after button press to ignore inputs
const unsigned short BUTTON_HOLD_TIME =        2000;                  // Hold time of status button to trigger reset
...
bool statusButtonReferenceSet =   false;                             // Used in detecting button holding
...
bool poisonCycleDone =            false;                             // Has the poison cycle been run
bool poisonCycleBlocking =        false;                             // Block poison cycle if setting time
bool highVoltage =                false;                             // Does high voltage need to be on
...
byte counterDigitsCycled =            0;                             // Digits cycled in cathode poisoning function
byte counterPoisonDigit =             0;                             // Currently selected digit in cathode poison function
...
unsigned long poisonCycleLast =       0;                             // When was poison cycle digit last incremented
...
void setup() {
     ...
     //Serial.begin(19200);
     //Serial.println("Nixie Clock Booting");

     bool initDone = false;
     byte nixieStorageInit = EEPROM.read(EEPROM_INITIALISED);        // Read initialised data from EEPROM
     
     //Serial.print("EEPROM Signiture At Boot: ");
     //Serial.println(nixieStorageInit, HEX);
     ...
     for (int i = 0; i < 6; i++) {
      ...
      nixieStorageInit = EEPROM.read(EEPROM_INITIALISED);
      
      if (nixieStorageInit == 0xAA && initDone) {
...
// =========================================================== Main program ============================================================ //
/* Order of functions in loop are critical. Standby isn't so critical, but putting it first makes sense. Multiplex could probably go
 * probably go elsewhere like the end of the loop. It must go after all the functions that can override timer(). Those functions do not
 * block timer from updating the display registers. Whatever timer does gets overidden by another function afterwards. The order of
 * overide functions has yet to be determined. It could be made that any one of them called overrides the last. I think cathode poisoning
 * should be the the lowest priority. If someone is interacting with the clock, having the display flash without prompt could be confusing.
 * The order of the other two is to decide. */
...
void loop() {
  ...
  if (!standbyDone && batteryBackup) {                               // Check to see if standby routine is done
    standby();
    digitalWriteFast(HV_POWER, LOW);
    } else if (!batteryBackup) {
      digitalWriteFast(HV_POWER, HIGH);
      standbyDone = false;
  }
  ...
  holdButtonHandler(STATUS_PIN, runTimeButtonState, currentButtonReferenceTime, runTimeButtonPrevPush);
  
  timer();
  
  if ((counterMinutes == 30 || counterMinutes == 0) && !poisonCycleDone && !poisonCycleBlocking) {
    cathodePoisoning();
  }
  hourModeDisplay();
  nixieRunTimeDisplay();
...
void buttonHandler(int avrPin, bool &buttonState, bool &prevButtonState, unsigned long currentReferenceTime, unsigned long &buttonPrevPush) {
   ...
   if (buttonState && !prevButtonState && (currentReferenceTime - buttonPrevPush > DEBOUNCE_DELAY)) {
     switch(avrPin) {
      case MINUTE_PIN:
        minuteButtonHandler();
        break;
...
void cathodePoisoning() {
  ...
  for (byte i = 0; i < 6; i++) {
    digits[i] = counterPoisonDigit;
  }
  
  if ((millis() - poisonCycleLast) >= DIGIT_DWELL_TIME) {
    
    counterPoisonDigit++;
    
    if (counterPoisonDigit > 9) {
      counterPoisonDigit = 0;
      counterDigitsCycled++;
    }
    
    if (counterDigitsCycled >= CATHODE_POISONING_REPEAT) {
      counterDigitsCycled = 0;
      counterPoisonDigit = 0;
      poisonCycleDone = true;
    }
    poisonCycleLast = millis();
  }
}
// ==================================================== Development debug ============================================================== //
...
void debug() {
    ...
    Serial.print("EEPROM Signiture Post Write: ");
    unsigned long temp1 = EEPROM.read(EEPROM_INITIALISED);
    Serial.println(temp1, HEX);
    Serial.print("EEPROM Run Time Hours: ");
    unsigned long temp2 = EEPROM.read(RUN_TIME_HOURS);
    Serial.println(temp2);

//  unsigned long temp = EEPROM.read(RUN_TIME_HOURS);
//  Serial.print("EEPROM Value Read Now: ");
//  Serial.println(temp, DEC);
//  temp++;
//  EEPROM.update(RUN_TIME_HOURS, temp);
...
void eepromRead() {
    ...
    Serial.print("EEPROM Value Read Now: ");
    Serial.println(nixieRunTime, DEC);    
}
// ========================================================== EEPROM Writes ============================================================ //
...
// =================================================== Hold button handler ============================================================= //
/* This function deals with the one button that is used for dual functions. It decides if the button is held down or if it has been
 * pressed. I'd like this merged with original button handling function. I think the best thing is to overwrite it completely with
 * this one. There is no need to track the previous state of the button here, holding down the button will not cause repeated triggers
 * as the button holding does something else completely and is dealt with. Switch case needs to be added. Buttons without other hold
 * functions can be added. Anything without a hold function can just trigger it's normal press function. */
void holdButtonHandler(int avrPin, bool &buttonState, unsigned long currentReferenceTime, unsigned long &buttonPrevPush) {

  buttonState = !digitalReadFast(avrPin);
  
  if (buttonState && !statusButtonReferenceSet && (currentReferenceTime - buttonPrevPush) > DEBOUNCE_DELAY) {
    heldTime = millis();
    statusButtonReferenceSet = true; 
  }
  if (buttonState && statusButtonReferenceSet && !holdToggled && (currentReferenceTime > (heldTime + BUTTON_HOLD_TIME))) {    
    holdToggled = true;
    resetHandler();
  }
  if (!buttonState && statusButtonReferenceSet && !holdToggled) { 
    statusButtonReferenceSet = false;
    eepromRead();
    showRunTime = true;
    showRunTimeStart = millis();
  } else if (!buttonState) {
...
void hourModeDisplay() {
    ...
    if (showHourMode) {
        if (twelveHourMode) {                                         // Display "12"
           digits[0] = INVALID_DATA;                                  // Invalid data causes the display driver to blank it's output
           digits[1] = INVALID_DATA;
           digits[2] = 1;
           digits[3] = 2;
           digits[4] = INVALID_DATA;
           digits[5] = INVALID_DATA;
        } else {                                                      // Display "24"
           digits[0] = INVALID_DATA;
           digits[1] = INVALID_DATA;
           digits[2] = 2;
           digits[3] = 4;
           digits[4] = INVALID_DATA;
...
void minuteButtonHandler() {
    ...
    if (!poisonCycleBlocking) {
      poisonCycleBlocking = true;
    }
}
 ...
 * from the modulo is assigned to the digit. */
...
void nixieRunTimeDisplay() {
    ...
    if (showRunTime) {                                                 // Don't read EEPROM each time as it's slow
      temp = nixieRunTime;                                             // Put value in another variable as this routine is destructive
        for (int i = 5; i >= 0; i--) {
           digits[i] = temp % 10;                                      // Divide nixieRunTime by ten, assign remainder to digit[i]
...
void timer() {
    ...
    if (counterMinutes != 30 && counterMinutes != 0 && (poisonCycleDone || poisonCycleBlocking)) {
      poisonCycleDone = false;
      poisonCycleBlocking = false;
    }
    ...
    if (counterHours >= 12 && counterHours <= 23)                      // Decide if it's AM or PM
    {             
      afternoon = true;
    } else {
      afternoon = false;
