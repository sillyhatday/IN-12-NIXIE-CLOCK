...
unsigned long minuteButtonPrevPush = 0;                             // time when minute button was pressed
unsigned long hourButtonPrevPush = 0;                               // time when hour button was pressed
...
void timer() {
    ...
    if (counterHours >= 24) {                                          // if 24 hours pass, wrap around
      counterHours = 0;
    }
...
void loop() {
  ...
  blinkColon();
    ...
    if (resetButtonState && !prevMinuteButtonState) {
      resetHandler();
