...
/* -----------------------------------------------------------------------------------------------------------------------------
  ...
  Code that is outside of the arduino C++ environment. Bare metal C! Exciting :)
...
//  buttonHandler(RESET_PIN,  resetButtonState,  prevResetButtonState,  currentButtonReferenceTime, resetButtonPrevPush);
  ...
  if (colonToggleFlag && (millis() - previousMillisColon >= 20)) {
    colonState = !colonState;
    previousMillisColon = millis();
    colonToggleFlag = false;
  }
...
//  colonState = !colonState;
  colonToggleFlag = true;                                             // Quickly set colon flag state and standby flag
}
// ========================================================= Button Handler ============================================================ //
// ------------------------------------------------------------------------------------------------------------------------------------- //
//
...
// The main display function. Changes here must be calculated. Small changes can screw up the display. Interrupts disabled until done.
// The colon state is worked out ahead of time by interrupt. Only do digitalWrite when needed to save more time.
...
void multiplex() {
  ...
  TIMSK &= ~(1 << OCIE1A);                                                // Disable interrupts else the display glitches when triggered
    ...
    if (colonOutputRequired != prevColonState) {
       digitalWrite(COLON_PIN, colonOutputRequired ? HIGH : LOW);
       prevColonState = colonOutputRequired;
    }
  ...
  TIMSK |= (1 << OCIE1A);                                                 // Reenable Interrupts
}
// =================================================== Show nixie run time ============================================================= //
