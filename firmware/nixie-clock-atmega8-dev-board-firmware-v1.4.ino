...
bool standbyDone =            false;                                 // has the stanby routine been performed
...
const int pwrPin =        13;                                        // pin to monitor if main power is removed
...
const unsigned int showModeDuration = 2000;                          // how long to show which hour mode we switched too
...
void setup() {
     ...
     for (int i = 0; i < 4; i++) {
        ...
        pinMode(pwrPin, INPUT);                                       // No pull up. Externally pulled down. 5v input means HIGH
...
//  batteryBackup = !digitalRead(pwrPin);                               // removed for development
  ...
  if (!batteryBackup && !showMode) {
    multiplex();
    blinkColon();
  } else if (!batteryBackup && showMode) {
    showModeMultiplex();
    blinkColon();
  }
