...
const uint8_t HOUR_PIN =                        2;                  // PC1 Hour increment
const uint8_t MINUTE_PIN =                      1;                  // PC2 Minute increment
...
const uint8_t DEBOUNCE_DELAY =                100;                  // Time after button press to ignore inputs
...
uint8_t standbyButtonState =                false;
uint8_t prevStandbyButtonState =            false;
...
void loop() {
  ...
  currentButtonReferenceTime = milliSecondCounter;              // Log time for comparing with last time button pressed

  standbyButtonState = !(PINC & (1 << PWR_PIN));                 // active low

  if (standbyButtonState != prevStandbyButtonState) {
    if (standbyButtonState) {
      standby();
    } else {
      wakeUp();
    }
    prevStandbyButtonState = standbyButtonState;
  }
  ...
  if (eepromWriteReady && currentMode != STANDBY_MODE) {
    eepromUpdate();
...
void eepromUpdate() {
  ...
  uint32_t runTimeEeprom = 0;
  
  EEPROM.get(RUN_TIME_HOURS, runTimeEeprom);
  runTimeEeprom++;
  EEPROM.put(RUN_TIME_HOURS, runTimeEeprom);
}
...
void multiplex()
{
  ...
  PORTD &= CATHODES_OFF;                       // Turn off all cathodes
