...
const int STATUS_PIN =                          A0;                  // For showing EEPROM time data, implement double function as reset
 ...
 * which hour mode has been selected. 12 hour or 24 hour mode. */
 ...
 * 
 * There are two multiplex routines. ShowHourMode or not. One is a 6 digit routine, one is a 2 digit routine. Simply skipping unused digits
 * causes the display brightness to increase. 2 digit routine wastes time on purpose to keep same duty cycle per digit. When all this
 * is completed, the digit can finally be switched on. The time between turning off a digit to turning it on is the most critical to
 * keep fast and short. When it's done, the next digit to display is recorded for next time. */
...
void multiplex() {
    ...
    if (micros() - previousMicros >= REFRESH_INTERVAL) {
       ...
       /*
        * Turn off only previous digit not all of them.
        * ~6000nS x6 = 36000nS = 360uS
        * ~6000nS x1 = 6000nS = 60uS
        * ~83% faster
        */
       ...
       if (!showHourMode) {                                               // If we aren't show the hour mode
         digitalWriteFast(DIGIT_BLANKING, HIGH);                          // Set CD4511 outputs ON
         ...
         if (currentDigit == 2 || currentDigit == 3) {                    // Only switch cathode on for middle digits
           ...
           digitalWriteFast(DIGIT_BLANKING, LOW);                         // If we are not on digits 2 or 3
           for (int i = 0; i < 4; i++) {                                  // Turn of each BCD output
             digitalWrite(BCD_PINS[i], LOW);                              // Time wasting, values don't matter
           }
         }
       }
